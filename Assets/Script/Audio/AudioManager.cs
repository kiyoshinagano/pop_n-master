...
public class AudioManager : SingletonMonoBehaviour<AudioManager>
{
    ...
    //↓(20161108) 追加------------------------------------↓
    private Dictionary<string, AudioClip> AudioClipSEDict = null;
    ...
    public void Awake ()
	{
        ...
        this.AudioClipSEDict = new Dictionary<string, AudioClip>();
        foreach (AudioClip se in Resources.LoadAll<AudioClip>("Audio/SE")){
            this.AudioClipSEDict.Add(se.name, se);
        }
        this.seAudioSource = this.gameObject.AddComponent<AudioSource>();
        //↑(20161108) 追加------------------------------------↑
    ...
    public void PlaySE(string seName){
        if (!this.AudioClipSEDict.ContainsKey(seName)){
            Debug.LogError(string.Format("SE名[{0}]が見つかりません。", seName));
            return;
        }

        this.seAudioSource.clip = this.AudioClipSEDict[seName];
        this.seAudioSource.Play();
    }

    public void StopSE(string seName){
        this.seAudioSource.Stop();
        this.seAudioSource.clip = null;
    }
    //↑(20161108) 追加------------------------------------↑
}
